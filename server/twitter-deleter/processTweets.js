...
async function processTweet(tweet) {
  tweet = tweet.hasOwnProperty('tweet') ? tweet.tweet : tweet
    
  // Check tweet structure
  let idProp
  let textProp

  if (tweet.hasOwnProperty('tweet_id')) {
    idProp = 'tweet_id'
  } else {
    idProp = 'id'
  }
  ...
  if (config.savedTweets.includes(tweet[idProp])) {
    console.log(`!!!! Tweet SAVED ${tweet[idProp]}\n${tweet[textProp]}`)

    await localStorage.setItem('lastTweetDeleted', tweet[idProp])

    return Promise.resolve(tweet)
  }

  return new Promise((resolve, reject) => {
    console.log(`Deleting tweet ${tweet[idProp]}\n${tweet[textProp]}`)

    /*
     * DEBUG TWEETS - gets them instead of deletes them
    return twitterClient.get(
      `statuses/show/${tweet[idProp]}`,
        ...
        if (err) {
          ...
          switch (errCode) {
            ...
            case 63:
              ...
              console.log(
                `You may have retweeted a user who has since been suspended.`
              )
            ...
            case 200:
              ...
              console.log(
                `Tweet is unavailable. TBH, not sure what this error means and it's undocumented by Twitter.`
              )
            ...
            default:
              console.error('Unhandled Twitter API error')
              console.log(err)
